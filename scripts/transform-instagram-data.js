...
const processInstagramData = async () => {
  ...
  for (let i = 0; i < instagramData.length; i++) {
    ...
    // Download images
    const productImages = [];
    const mainImages = post.images && post.images.length > 0 ? post.images : [post.displayUrl];

    for (let j = 0; j < Math.min(mainImages.length, 3); j++) {
      if (!mainImages[j]) continue;

      try {
        const imageHash = crypto.createHash('md5').update(mainImages[j]).digest('hex').substring(0, 8);
        const filename = `${post.shortCode}_${j + 1}_${imageHash}.jpg`;
        const filepath = path.join(imagesDir, filename);
        const publicPath = `/images/products/${filename}`;

        console.log(`  Downloading image ${j + 1}...`);
        await downloadImage(mainImages[j], filepath);
        productImages.push(publicPath);
    ...
    if (productImages.length === 0) {
      console.log(`  ⚠ No images downloaded for ${post.shortCode}, skipping product`);
      continue;
    }
    ...
    if (i % 10 === 0 && i > 0) {
      console.log(`Processed ${i} items, pausing...`);
  ...
  console.log(`📸 Images downloaded: ${products.reduce((sum, p) => sum + p.images.length, 0)}`);
  console.log(`💾 Data saved to: data/boom-warehouse-products.json`);
};

// Run the processing
processInstagramData().catch(console.error);