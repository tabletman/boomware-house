#!/usr/bin/env bash
# setup-warp.sh -- Create Warp workflow, health-check script, and optional docs.
# Usage:
#   ./scripts/setup-warp.sh           # write files (no git commit)
#   ./scripts/setup-warp.sh --commit "Commit message"  # create files and commit
#   ./scripts/setup-warp.sh --docs   # also create docs/WARP_SETUP.md and docs/GPT5_CODEX_PROMPT.md
#
# Idempotent: if a file exists it will be backed up with a timestamp suffix.
set -euo pipefail
...
while [ $# -gt 0 ]; do
  case "$1" in
    --commit)
      shift
      if [ $# -eq 0 ]; then
        echo "--commit requires a commit message" >&2
        exit 2
      fi
      COMMIT_MSG="$1"
      shift
    ...
    --docs)
      ...
      ;;
    --help|-h)
      echo "Usage: $0 [--commit \"message\"] [--docs]"
      exit 0
      ;;
    *)
...
# 1) Create .warp/workflow_frontier.json
WORKFLOW_PATH=".warp/workflow_frontier.json"
WORKFLOW_CONTENT='{
...
# 2) Create scripts/warp-frontier-check.sh
SCRIPT_PATH="scripts/warp-frontier-check.sh"
...
# 3) Optionally write docs
if [ "$MAKE_DOCS" -eq 1 ]; then
  DOC1_PATH="docs/WARP_SETUP.md"
  DOC1_CONTENT='# Warp — Setup, Workflows, and Command Center Integration
...
- A Warp workflow should include environment hints and steps. See `.warp/workflow_frontier.json` in the repo for an example (provided by this installer).
