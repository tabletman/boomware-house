...
1) Ingest and enforce repository boilerplate
   - Read and obey the repository's authoritative docs and boilerplate: docs/README.md, WARP.md or WARP.md variants, docs/WARP_SETUP.md, docs/GPT5_AGENTWISE_PROMPT.md, docs/GPT5_CODEX_PROMPT.md, tools/ wrapper conventions, .warp manifests, logs/tool-invocations.log format, and any other docs under docs/.
...
4) Proceed with development (authorization)
...
5) Idempotency, safety, and auditability (must implement)
...
6) Task lifecycle and orchestration outputs
   ...
   - For queue jobs (BullMQ + Redis), include payload schema, queue name, priority, TTL, retry policy, and dead-letter guidance, plus wrapper line to enqueue.

7) Warp integration & file outputs
...
8) Knowledge graph, observability & CI
   - Produce or update `docs/KNOWLEDGE_GRAPH.json` (JSON‑LD) following repo naming conventions.
   - Provide a minimal telemetry plan: metrics names (queue_length, job_latency_seconds, wrapper_invocations_total, job_failures_total).
...
10) Output & validation rules
   ...
   - After each file block include a 1–3 line purpose summary and a single smoke-test command (for example: `sh -n tools/run-bullmq.sh` or `jq . .warp/workflow_frontier.json`).
   - Provide git commands to create a branch `chore/agentwise-<timestamp>`, add files, and commit (do not push or open PRs without explicit authorization).

11) Acceptance criteria (success)
   ...
   - Provide CI snippet and knowledge graph update.
...
Smoke-test examples (must accompany generation)
- Validate a wrapper:
...
Note: You are authorized to proceed with development as described above: generate files, wrappers, workflows, scripts, and docs (including this `WARP.md`) and to run non‑destructive dry-run validations. Request secrets only when required to complete networked or privileged actions. Proceed.
